...
/* EB 29/6/2017 9286 START */

function getProgStart(thisForm, servicePath) {
    fillProgStart(thisForm.p_start_my, thisForm, servicePath);
}

function fillProgStart(frmObj, thisForm, servicePath) {
    // This function executes the makeRequest function that 
    // connects to the server via the URL.
    term = thisForm.p_termcode.value;
    program = thisForm.p_programme.value.substr(thisForm.p_programme.value.indexOf("|") + 1, thisForm.p_programme.value.lastIndexOf("|") - (thisForm.p_programme.value.indexOf("|") + 1));
    mode = thisForm.p_mode_entry.value.substr(0,thisForm.p_mode_entry.value.indexOf("/") );
    mode = mode.replace(/(\r\n|\n|\r)/gm,"");
    //alert("thisForm.p_mode_entry.value "+thisForm.p_mode_entry.value);
    
    
    //alert("term "+term+" /  program "+ program +" /  mode "+ mode);
    url = servicePath + 'hwzkclof.p_getProgStartMonthYear?pTerm=' + escape(term) + '&pProgram=' + program + '&pMode=' + escape(mode) + '&pEarliestStart=Y';
    makeRequest(url, frmObj, 'retrieveOptions');
}

function getAltProgStart(thisForm, servicePath) {
    //alert('getAltProgStart');
    fillAltProgStart(thisForm.p_altmonth_year, thisForm, servicePath);
    //alert('thisForm.p_altmonth_year.value '+thisForm.p_altmonth_year.value);

}

function fillAltProgStart(frmObj, thisForm, servicePath) {
    // This function executes the makeRequest function that 
    // connects to the server via the URL.
    term = thisForm.p_termcode.value;
    program = thisForm.p_altprogramme.value.substr(thisForm.p_altprogramme.value.indexOf("|") + 1, thisForm.p_altprogramme.value.lastIndexOf("|") - (thisForm.p_altprogramme.value.indexOf("|") + 1));
    mode = thisForm.p_altmode_altentry.value.substr(0,thisForm.p_altmode_altentry.value.indexOf("/") );
    mode = mode.replace(/(\r\n|\n|\r)/gm,"");
     

    url = servicePath + 'hwzkclof.p_getProgStartMonthYear?pTerm=' + escape(term) + '&pProgram=' + program + '&pMode=' + escape(mode) + '&pEarliestStart=N';
    //alert(url);
    makeRequest(url, frmObj, 'retrieveOptions');
}

function setTranscriptYes(thisForm) {


  var nationsUL = "-5826-8826-7826-2826-6826-1593-1594-1592-1599-";
  var selectedNation = "-"+thisForm.p_szrclof_nationality.value+"-";

  if (isNaN(thisForm.p_szrclof_tariff.value)|| (nationsUL.indexOf(selectedNation) <0&&selectedNation !== "--")) {
    thisForm.p_szrclof_transcripts.value = "Y";
  } else {
    thisForm.p_szrclof_transcripts.value = "";
  }
}

/* EB 29/6/2017 9286 END */
//////////////////////////////////////////////////////////////


const doFetch = (urlName, callBack) => {
	
    const databaseInstance = SingletonDatabase.getDatabaseInstance();
    const databaseConnectInstance = createDatabaseConnection (urlName, 
	                                                          callBack, 
														      null, 
														      databaseInstance);
    databaseConnectInstance.sendAndFetchData();
	
}

const checkProgAvailability = (progChoice,thisForm) => {
   
   /* 1) get the programme/start month/start year from the clearing form
      2) make a http request to the procedure: hwzkclof.p_check_prog_availability 									   
      3) if a text is populated then show it in the the prog availability div */

    const progAvailability = progChoice==="choice" ? $('#progAvailability') : $('#altProgAvailability');    
    progAvailability.html(""); 
    progAvailability.removeClass('prog-availability-class'); 
	   												 
    const progSelectVal = progChoice==="choice" ? thisForm.p_programme.value : thisForm.p_altprogramme.value; 
    const progCode = progSelectVal.substr(progSelectVal.indexOf("|") + 1, progSelectVal.lastIndexOf("|") - (progSelectVal.indexOf("|") + 1));
    const monthYear = progChoice==="choice" ? thisForm.p_start_my.value : thisForm.p_altmonth_year.value; 
    const startMonth = monthYear.substr(0,monthYear.indexOf("/"));  
    const startYear = monthYear.substr(monthYear.indexOf("/")+1);  
    const urlName = encodeURIComponent('hwzkclof.p_check_prog_availability')+ "?" + "p_prog_code=" + progCode + "&p_start_month=" + startMonth + "&p_start_year=" + startYear;

    doFetch(urlName,(string)=>{if (string) {
                                           progAvailability.html(string);
						 progAvailability.addClass('prog-availability-class'); 
						 }
				   });
